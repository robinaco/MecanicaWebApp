            ...
            <div class="card bg-ligth bg-gradient border border-secondary">
                <div class="card-body">
                    <i><span class="badge bg-danger bg-gradient">Campos marcados con * son obligatorios.</span></i>
                        ...
                        <form class="row g-3" action="SaveLatoneria" method="POST" enctype="multipart/form-data">
                            @csrf
                            <div class="col-md-1">
...
<script type="text/javascript">
 $('#registrar').attr('disabled', true);
        function agregarProducto() {
            ...
            newtr = newtr + '<td class="iProduct"><input type="hidden" value="{{$vehiculo->id}}" name="idvehiculo[]" id="idvehiculo"class="form-control" readonly> <input type="hidden" value="{{$orden}}" name="odsl[]" id="odsl" class="form-control" readonly> <select name="pro_id[]" id="pro_id" class="selectpicker form-control border border-success border-gradient border-gradient" style="background-color:yellow">'
            newtr = newtr +  '<option value="Mecanica General"><b>Mecanica General</b></option>'
            ...
            newtr = newtr + '<td><input class="form-control border border-success border-gradient border-gradient" type="text"   id="concept[]" name="concept[]"/></td>';
            newtr = newtr + '<td><input class="form-control border border-success border-gradient border-gradient" type="number"  min="1" id="cantidad[]" name="cantidad[]" onChange="Calcular(this);" value="1" /></td>';
            newtr = newtr + '<td><input class="form-control border border-success border-gradient border-gradient" type="number" min="0" id="precunit[]" name="precunit[]" onChange="Calcular(this);" value="0"/></td>';
            newtr = newtr + '<td><input class="form-control border border-success border-gradient border-gradient" type="text" id="totalitem[]" name="totalitem[]" readonly /></td>';
            newtr = newtr + '<td><button type="button" class="btn btn-danger btn-sm remove-item" ><i class="fa fa-times" title="Borrar Fila"></i></button> <button type="button" onclick="agregarProducto()" class="btn btn-primary  btn-sm" title="Agregar Fila"><i class="fa fa-plus"></i></button></td></tr>';
        ...
        function Calcular(ele) {
            ...
            var total = document.getElementById("total");
               ...
               if (total.innerHTML == 'NaN') {
                ...
                document.getElementById('cost').value = total.innerHTML; 
                document.getElementById("resta").value = total.innerHTML; 
                total.innerHTML = 0;
               }else{
                total.innerHTML = parseFloat(total.innerHTML)+ totalitem - anterior ;   
                document.getElementById('cost').value = total.innerHTML; 
